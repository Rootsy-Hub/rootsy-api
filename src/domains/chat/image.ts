...
const CHAT_IMAGE_STORAGE_BUCKET = "rootsy_catalog_public"
const MAX_BYTES = 5 * 1024 * 1024

function buildChatImageFileName(): string {
  return `${crypto.randomUUID()}.webp`
}

function buildChatImageStoragePath(popId: string, fileName: string): string {
  return `${popId}/chat/${fileName}`
}
...
> {
  ...
  const fileName = buildChatImageFileName()
  const storagePath = buildChatImageStoragePath(popId, fileName)
  const bytes = Buffer.from(await file.arrayBuffer())

  const { error: uploadError } = await supabase.storage
    .from(CHAT_IMAGE_STORAGE_BUCKET)
